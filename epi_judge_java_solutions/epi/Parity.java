...
public class Parity {
    ...
    /**
     * 思路二：利用异或的特性（1 ^ 1 = 0, 0 ^ 1 = 1; 1 ^ 0 = 1, 0 ^ 0 = 0;），直接计算奇偶性
     * <p>
     * 时间复杂度：O(n)，n 为 64
     */
    ...
    /**
     * 思路三：利用清最低位 1 的特性（x & (x - 1)），跳过值为 0 的位
     * <p>
     * 时间复杂度：O(k)，k 为 1 的个数
     */
    ...
    /**
     * 思路四：利用异或的特性（交换律和结合律），每次将高位部分与低位部分进行异或，结果存在低位，直到只剩 1 位
     * <p>
    ...
    /**
     * 思路五：利用异或的特性（结合律），将 long 拆为 4 个 16 位数，分别计算它们的奇偶性，最后再对 4 个数的奇偶性进行异或
     * <p>
